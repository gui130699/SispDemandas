...
export const defaultStatuses: Omit<DemandStatus, "id">[] = [
  { name: "Em análise", description: "Demanda registrada e aguardando avaliação.", color: "#3b82f6", order: 10, active: true, legacyKeys: ["new", "triage", "analysis"] },
  ...
  { name: "Pausada", description: "Execução temporariamente pausada.", color: "#64748b", order: 60, active: true, legacyKeys: ["paused"] },
  { name: "Concluída", description: "Demanda finalizada.", color: "#16a34a", order: 90, active: true, isFinal: true, legacyKeys: ["completed"] },
  { name: "Cancelada", description: "Demanda cancelada.", color: "#dc2626", order: 100, active: true, isFinal: true, legacyKeys: ["cancelled"] },
];
...
export function canChangeDemandStatus(user: UserProfile) { return user.role === "admin" || (user.role === "consultant" && user.permissions?.canChangeStatus !== false); }
